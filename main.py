...
def get_predictions_for_latest_season():
...
def get_predictions_based_on_old_RPDEF():
    ...
    # Get the data for the 2022/23 season
    X_test, _ = get_data_for_new_season()
    X_test = np.array(X_test).reshape(-1, 1)
    # Add the year as a feature for the test data
    num_test_samples = len(X_test)
    years_test = np.array([2023] * num_test_samples).reshape(-1, 1)
...
def get_data_for_new_season():
    ...
    mycursor = mydb.cursor(dictionary=True)
    mycursor.execute(f"SELECT AVG(P21.DEF) AS RPDEF_21_22 FROM Player AS P22 JOIN Player AS P21 ON P22.Player = P21.Player WHERE P22.SeasonYear = '22/23' AND P21.SeasonYear = '21/22' AND P21.DEF > 0 GROUP BY P22.Team ORDER BY P22.Team")
    ratings = mycursor.fetchall()
    for rating in ratings:
...
get_predictions_for_latest_season()
print("--------------------------------")
